...
def tfromfile(name):
    ...
    # # Splitting the input string based on underscores
    # split_string = name.split('_')
    #
    # prefix = '_'.join(split_string[:-2])  # Prefix
    # second_set = split_string[-2]  # Second set of digits
    # third_set = split_string[-1].replace('.dat.gz', '')  # Third set of digits
    # fourth_element = '1000.dat.gz'  # Assuming a default value for the missing fourth element
    #
    # parts = [prefix, second_set, third_set, fourth_element]

    print('parts', parts)
    y, m, d = list(map(int, (parts[-3][0:2], parts[-3][2:4], parts[-3][4:6])))
    H, M, S = list(map(int, (parts[-2][0:2], parts[-2][2:4], parts[-2][4:6])))

    print('1', list(map(int, (parts[-3][0:2], parts[-3][2:4], parts[-3][4:6]))))
    print('2', list(map(int, (parts[-2][0:2], parts[-2][2:4], parts[-2][4:6]))))
...
def generate_nc_for_nalma(BASE_PATH):
    params = {'stations':(6,99), # range of allowable numbers of contributing stations
              'chi2':(0,1.0),    # range of allowable chi-sq values
              'distance':3000.0, 'thresh_critical_time':0.15, # space and time grouping thresholds
              'thresh_duration':3.0, # maximum expected flash duration
              'ctr_lat':34.7246100, 'ctr_lon':-86.6453300, #center lat/lon to use for flash sorting, gridding
    ...
    print(filenames)
    # filenames = ['data/input/LC_NALMA_230704_220000_0100.dat.gz']
    # filenames = ['data/input/NALMA_200305_142000_0600.dat.gz']
    # filenames = ['data/input/NALMA_230704_171000_0600.dat.gz']
